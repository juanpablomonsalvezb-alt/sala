  ...
  return text
    ...
    .map((line) => {
      ...
      const processed = line
        ...
        .replace(/\*(.+?)\*/g, "<em>$1</em>")
        .replace(/\[(.+?)\]\((.+?)\)/g, '<a href="$2" style="color:#C41C1C;text-decoration:underline">$1</a>');
...
export default function NuevaPublicacionPage() {
  ...
  const handlePublish = () => {
    if (!title.trim() || !body.trim()) {
      alert("El título y el contenido son obligatorios para publicar.");
      return;
    }
  ...
  return (
    ...
    >
      {/* ── SIDEBAR (mini, sin estado activo) ─────────────────────────────── */}
      <aside className="w-[220px] flex-shrink-0 bg-white border-r border-[#DEDEDE] flex flex-col h-full">
        ...
        <div className="px-5 py-5 border-b border-[#DEDEDE]">
          <div className="w-9 h-9 rounded-full bg-[#121212] flex items-center justify-center mb-3">
            <span className="text-white text-[13px] font-medium">RF</span>
          </div>
          <p className="text-[13px] font-semibold text-[#121212] leading-snug">
            Rodrigo Fuentes
          </p>
          <a
        ...
        <nav className="flex-1 py-3">
          {[
            ...
            <Link
              key={item.label}
              href={item.href}
              className="w-full flex items-center gap-3 px-5 py-2.5 text-[#666666] hover:text-[#121212] hover:bg-[#F7F7F7] transition-colors"
            >
        ...
        <div className="px-5 py-4 border-t border-[#DEDEDE]">
          ...
          <button className="mt-1.5 text-[12px] text-[#C41C1C] font-medium hover:underline">
            Actualizar plan
          </button>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto">
          {preview ? (
            ...
            <div className="max-w-[680px] mx-auto py-14 px-6">
              ...
              >
                {title || (
                  <span className="text-[#DEDEDE]">El título de tu análisis...</span>
                )}
              </h1>
              ...
              <div className="flex items-center gap-3 pb-6 border-b border-[#DEDEDE] mb-8">
                <div className="w-8 h-8 rounded-full bg-[#121212] flex items-center justify-center">
                  <span className="text-white text-[11px] font-medium">RF</span>
                </div>
                <div>
                  <p className="text-[13px] font-semibold text-[#121212]">Rodrigo Fuentes</p>
                  <p className="text-[11px] text-[#666666]">8 de mayo de 2025 · Análisis financiero</p>
                </div>
              </div>
        ...
        <div className="flex-shrink-0 bg-white border-t border-[#DEDEDE] px-8 py-4 flex items-center justify-between">
          ...
          >
            {body.trim()
              ? `${body.trim().split(/\s+/).length} palabras`
              : "Sin contenido aún"}
          </span>
          ...
          <div className="flex items-center gap-3">
            {saved && (
              <span
                className="text-[12px] text-[#666666] animate-fade-in"
                style={{ fontFamily: "var(--font-inter), Inter, sans-serif" }}
              >
                ✓ Borrador guardado
              </span>
            ...
            <button
              ...
              className="text-[13px] font-medium text-[#121212] border border-[#DEDEDE] px-4 py-2 hover:border-[#666666] hover:bg-[#F7F7F7] transition-colors"
              style={{ fontFamily: "var(--font-inter), Inter, sans-serif" }}
            >
              Guardar borrador
            </button>
            <button
              ...
              className="text-[13px] font-semibold text-white bg-[#121212] px-5 py-2 hover:bg-[#2a2a2a] transition-colors"
              style={{ fontFamily: "var(--font-inter), Inter, sans-serif" }}
            >
              Publicar →
            </button>
